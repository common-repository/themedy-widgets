 ...
 * Enqueue CSS
 */
add_action('init', 'themedy_video_enqueue');
function themedy_video_enqueue() {
	/* Paths */
	$themedy_widgets_path = dirname(__FILE__);
	$themedy_widgets_main_file = dirname(__FILE__).'/widget-video.php';
	$themedy_widgets_directory = plugin_dir_url($themedy_widgets_main_file);

	if (is_active_widget(false, false, 'themedy_video_widget') and !is_admin()){
		wp_enqueue_style('themedy-video-css', $themedy_widgets_directory.'css/widget-video.css');
	}
}
